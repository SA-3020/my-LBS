...
class LastLocationAdapter(var mContext: Context, var data_list: List<Locations>) :
    RecyclerView.Adapter<LastLocationAdapter.ViewHolder>() {
    inner class ViewHolder(v: View) : RecyclerView.ViewHolder(v), View.OnClickListener {
        var location_textView: TextView
        var sub_location_textView: TextView
        var item: Locations? = null
        override fun onClick(v: View) {
            clickListener!!.onItemClick(adapterPosition, v, 2)
        }
